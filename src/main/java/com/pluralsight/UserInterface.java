...
public class UserInterface {
    ...
    private void addPizza() {
        ...
        if (ConsoleHelper.promptForYesNo("Add sauces?")) {
            IntStream.range(0, sauces.length)
                    .filter(i -> ConsoleHelper.promptForYesNo("Add" + sauces[i] + "?"))
                    .forEach(i -> pizza.addSauce(new Sauce(sauces[i])));
    ...
    private void addDrink(){
        ...
        String[] drinks = {"1) Small","2) Medium","3) Large"};
        double[] prices = {2.00,2.50,3.00};

        IntStream.range(0,drinks.length)
